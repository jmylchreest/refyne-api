---
title: Rust SDK
description: Official Rust client with async tokio support
---
...
async fn main() -> Result<(), refyne::Error> {
    ...
    let result = client.extract(ExtractRequest {
        url: "https://demo.refyne.uk/products/5".into(),
        schema: json!({
            "name": "string",
            "price": "number",
            "description": "string",
...
async fn main() -> Result<(), refyne::Error> {
    ...
    let analysis = client.analyze(AnalyzeRequest {
        url: "https://demo.refyne.uk/products".into(),
        depth: Some(1),
    }).await?;
...
async fn main() -> Result<(), refyne::Error> {
    ...
    let job = client.crawl(CrawlRequest {
        ...
        options: Some(CrawlOptions {
            follow_pattern: Some("/products/[0-9]+".into()),
            max_pages: Some(20),
            ..Default::default()
        }),
        ..Default::default()
